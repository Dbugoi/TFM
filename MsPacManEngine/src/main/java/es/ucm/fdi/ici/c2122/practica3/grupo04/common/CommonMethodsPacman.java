package es.ucm.fdi.ici.c2122.practica3.grupo04.common;
...
public class CommonMethodsPacman {
	...
	public static MOVE avoidGhosts(Game game, int currPos, int goalPos) {
		...
		return game.getNextMoveTowardsTarget(currPos, CommonMethodsPacman.findClosestPill(game, currPos), Constants.DM.PATH);
	}
	...
	// Encuentra la pill mas cercana 
	public static int findClosestPill(Game game, int pos) {
		int[] pills = game.getActivePillsIndices();
		return findClosestPill(game, pos, pills);
	}

	// Encuentra la pill mas cercana 
	public static int findClosestPill(Game game, int pos, int[] pills) {
		int nearestPill = -1;
		int nearestDistance = Integer.MAX_VALUE;
		for(int i = 0; i < pills.length; ++i) {
			int dist = game.getShortestPathDistance(pos, pills[i],game.getPacmanLastMoveMade());
