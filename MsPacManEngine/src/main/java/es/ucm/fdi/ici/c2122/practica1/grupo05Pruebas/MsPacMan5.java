...
public final class MsPacMan5 extends PacmanController {
	...
	public MOVE getMove(Game game, long timeDue) {
		...
		if (nearestChasingGhost!=null && nearestEdibleGhost!=null)
		{
			...
			else if(game.getGhostEdibleTime(nearestEdibleGhost)>= distanceEdibleGhost_MsP&&(distanceEdibleGhost_MsP<distanceEdibleGhost_ChasingPacman  || distanceEdibleGhost_MsP< distanceChasingGhost_MsP) && distanceChasingGhost_MsP >40 && dangerCage==-1)
			...
			else if (distanceChasingGhost_MsP < limit/2)
				return game.getNextMoveAwayFromTarget(game.getPacmanCurrentNodeIndex(), indexChasingGhost, DM.PATH);
			//else
		...
		else if (nearestChasingGhost!=null )
		{
			...
			if(dangerCage!=-1) 
				return game.getNextMoveAwayFromTarget(game.getPacmanCurrentNodeIndex(), 492, DM.PATH);
			else if(indexPpill!=-1&&(distancePPill_MsP < distanceChasingGhost_MsP) && distancePPill_ChasingGhost<= distancePPill_MsP*2.8) //PP + cerca, capaz de comer Chasing
	...
	private MOVE getBestRunAwayGhost( Game game, int limit, int distasncePPill, int indexNearestChasingP) {
		...
		for(MOVE nextMove: m) {
			...
			for(Constants.GHOST g: Constants.GHOST.values()) {
				int ghost = game.getGhostCurrentNodeIndex(g);
				if(!game.isGhostEdible(g) && game.getShortestPathDistance(newNodeMsPacMan, ghost)<=limit /*/2+20*/)
		...
		if(indexNearestChasingP!=-1)
			if(game.getNodeYCood(mspacman)<20 && game.isJunction(mspacman) && distasncePPill>limit && game.getNextMoveTowardsTarget(mspacman, indexNearestChasingP, DM.PATH)!=game.getNextMoveTowardsTarget(game.getPacmanCurrentNodeIndex(),492, DM.PATH) )
				finalMove = game.getNextMoveTowardsTarget(game.getPacmanCurrentNodeIndex(),492, DM.PATH);
		
		
		if(game.getActivePowerPillsIndices().length==0 ||distance == Integer.MIN_VALUE)
			finalMove = game.getNextMoveTowardsTarget(mspacman, getNearestPillWithoutChasingGhost(game, indexNearestChasingP), DM.PATH);
		return finalMove;
	}
	...
	private MOVE getBestMoveTowardsGhost(Game game, int limit, int indexNearestEdible) {
		...
		for(MOVE nextMove: m) {
			...
			for(Constants.GHOST g: Constants.GHOST.values()) {
				int ghost = game.getGhostCurrentNodeIndex(g);
				if(game.isGhostEdible(g) &&game.getShortestPathDistance(newNodeMsPacMan, ghost) <=limit /*/2+10*/)
					d += game.getShortestPathDistance(newNodeMsPacMan, ghost);
	...
	private MOVE avoidCornersWithoutPPill(Game game, MOVE m, int distance_PPill, int limit) {
		int index = game.getPacmanCurrentNodeIndex();
		
		int p1= game.getShortestPathDistance(index, 97),p2 = game.getShortestPathDistance(index, 102),
		 p3 = game.getShortestPathDistance(index, 1143), p4 = game.getShortestPathDistance(index, 1148);
		if(game.isJunction(index) && p1<=30  ) {
			return game.getNextMoveAwayFromTarget(game.getPacmanCurrentNodeIndex(), 97, DM.PATH); //ir al centro
		}
		else if(game.isJunction(index) && p2<=30  ) {
			return game.getNextMoveAwayFromTarget(game.getPacmanCurrentNodeIndex(), 102, DM.PATH); //ir al centro
		}
		else if(game.isJunction(index) && p3<=30  ) {
			return game.getNextMoveAwayFromTarget(game.getPacmanCurrentNodeIndex(), 1143, DM.PATH); //ir al centro
		}
		else if(game.isJunction(index) && p2<=30  ) {
			return game.getNextMoveAwayFromTarget(game.getPacmanCurrentNodeIndex(), 1148, DM.PATH); //ir al centro
		}
		else
			return m;
	}
