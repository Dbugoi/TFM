...
public class RunAwayFromMsPacMan_GhostsAction implements RulesAction {

	GHOST g;
	DM dm =  DM.PATH;
	int threshold = 90;
	public RunAwayFromMsPacMan_GhostsAction(GHOST ghost) {
		this.g = ghost;
	}
	...
	public MOVE execute(Game game) {
		if (game.doesGhostRequireAction(g))        //if it requires an action
		{
			...
			if(nearest_ghost != null) {
				...
				return calculaMovimientoSeguro(distance,runaway_pacman_move,runaway_ghost_move);
			}
			else
	...
	private MOVE calculaMovimientoSeguro(int distance_to_pacman,MOVE runaway_pacman_move, MOVE runaway_ghost_move) {
	...
	public String getActionId() {
		return g+ "runsAway from MsPacMan and other ghosts";
	}
