...
public class Ghosts1 extends GhostController {
    ...
    public EnumMap<GHOST, MOVE> getMove(Game game, long timeDue) {
        ...
        for (GHOST ghost : GHOST.values()) {
            ...
            else if (isThereAnEdibleGhost(game))
                move = moveTowardsClosestEdibleGhost(game, ghost);
    ...
    private boolean isThereAnEdibleGhost(Game game) {
        return Arrays.stream(GHOST.values()).anyMatch(game::isGhostEdible);
    }
    ...
    private MOVE moveTowardsClosestEdibleGhost(Game game, GHOST ghost) {
        int ghostIndex = game.getGhostCurrentNodeIndex(ghost);
        MOVE lastMove = game.getGhostLastMoveMade(ghost);
        ToIntFunction<GHOST> distToGhost =
                g -> (int) game.getDistance(ghostIndex, game.getGhostCurrentNodeIndex(g),
        ...
        Optional<GHOST> closestNonEdibleGhost =
                Arrays.stream(GHOST.values()).filter(game::isGhostEdible)
                        .sorted((a, b) -> distToGhost.applyAsInt(a) - distToGhost.applyAsInt(b))
                        .findFirst();
