 ...
 * <ul>
 * <li>By default it only can manage a few data types, although developers can add their own ones
 * implementing the TypeAdaptor interface.<br>
 * Supported types and the type extension mechanism is explained in PlainTextTypeConverter.
 * <li>Only works with one file.
 * </ul>
 * <p>
 * This connector uses the property in the initFromXMLfile() parameter to obtain the configuration
 * file. This file is a xml that follows the Schema defined in
 * <a href="PlainTextConnector.xsd">/doc/configfilesSchemas/PlainTextConnector.xsd</a>:
 * <p>
 * <img src="PlainTextConnectorSchema.jpg">
 * <p>
 * This class does not implement any cache mechanims, so cases are read and written directly. This
 * can be very inefficient in some operations (mainly in reading)
 * <p>
 * Some methods will fail when executing the connector with a case base file inside a jar file. The
 * retrieve() methods will work properly but the methods that write in the file will fail. Extract
 * the file to the file system and run the connector with that location to solve these problems.
 * <p>
...
public class CustomPlainTextConnector implements Connector {
	...
	public void initFromXMLfile(URL file) throws InitializingException {
		try {
			...
			this.descriptionClass = Class.forName(
					doc.getElementsByTagName("DescriptionClassName").item(0).getTextContent())
					.asSubclass(CaseComponent.class);

			try {
				this.solutionClass = Class.forName(
						doc.getElementsByTagName("SolutionClassName").item(0).getTextContent())
						.asSubclass(CaseComponent.class);
			} catch (Exception e) {
			}
			...
			try {
				this.justOfSolutionClass =
						Class.forName(doc.getElementsByTagName("JustificationOfSolutionClassName")
								.item(0).getTextContent()).asSubclass(CaseComponent.class);
			} catch (Exception e) {
			}

			try {
				this.resultClass = Class.forName(
						doc.getElementsByTagName("ResultClassName").item(0).getTextContent())
						.asSubclass(CaseComponent.class);
			} catch (Exception e) {
			}
			...
			if (this.solutionClass != null)
				this.solutionMaps = findMaps(doc.getElementsByTagName("SolutionMappings").item(0),
						this.solutionClass);

			if (this.justOfSolutionClass != null)
				this.justOfSolutionMaps = findMaps(
						doc.getElementsByTagName("JustificationOfSolutionMappings").item(0),
						this.justOfSolutionClass);

			if (this.resultClass != null)
				this.resultMaps = findMaps(doc.getElementsByTagName("ResultMappings").item(0),
						this.resultClass);
	...
	/**
	 * Simple extension to allow custom case base files. It also creates a new empty file if it does
	 * not exist.
	 */
	...
	/**
	 * Stores the cases in the data base. Note that this method does not control that the case name
	 * (== primary key) is repeated, so developers must be careful with this.
	 * 
	...
	public void storeCases(Collection<CBRCase> cases) {
		try {
			...
			br = new BufferedWriter(
					new FileWriter(FileIO.findFile(this.PROP_FILEPATH).getFile(), true));
	...
	private void writeComponent(CaseComponent comp, List<Attribute> maps, StringBuffer line,
			char separator, boolean includeId) {
		try {
			if (includeId)
	...
	/**
	 * Deletes cases from the case base. It only uses the case name (primary key) to remove the row.
	 * Note that this method is very inefficient because it reads all the database, removes the rows
	 * in memory, and writes it again into the text file.
	 * 
	...
	public void deleteCases(Collection<CBRCase> cases) {
		try {
			...
			br = new BufferedReader(
					new InputStreamReader(FileIO.findFile(this.PROP_FILEPATH).openStream()));
			...
			bw = new BufferedWriter(
					new FileWriter(FileIO.findFile(this.PROP_FILEPATH).getFile(), false));
	 ...
	 * in memory, and writes it again into the text file.
	 * 
	 * @param casesIds Cases to delete
	 */
	public void deleteCases(Set<Integer> casesIds) {
		try {
			...
			try (BufferedReader br = new BufferedReader(
				...
				while ((line = br.readLine()) != null) {
					...
					StringTokenizer st = new StringTokenizer(line, this.PROP_DELIM);
					int caseId = Integer.parseInt(st.nextToken());
					if (!casesIds.contains(caseId))
						lines.add(line);
				}
			}

			// clear file
			new PrintWriter(FileIO.findFile(this.PROP_FILEPATH).getFile()).close();
	...
	/**
	 * Retrieves all cases from the text file. It maps data types using the PlainTextTypeConverter
	 * class.
	 * 
			...
			while ((line = br.readLine()) != null) {
				...
				if (this.justOfSolutionClass != null) {
					CaseComponent justificationOfSolution =
							(CaseComponent) this.justOfSolutionClass.newInstance();
	...
	private void fillComponent(CaseComponent component, StringTokenizer st, List<Attribute> maps,
			boolean includeId) {
		try {
			Class<?> type;
			Object value;
