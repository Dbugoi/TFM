...
public class Paths {
    ...
    /**
     * Devuelve {@code true} si hay un fantasma que está en {@code path}.
     * @param game
     * @param path
     * @return
     */
    public static boolean isAnyGhostInPath(Game game, int[] path){
        return Arrays.stream(GHOST.values())
            .anyMatch(gh -> isGhostInPath(game, gh, path));
    }
    ...
    public static int getDestinationJunctionForMsPacman(Game game) {
        return getDestinationJunctionForTarget(game,
    ...
    public static int getDestinationJunctionForTarget(Game game, int node, MOVE lastMove) {
        ...
        for (int junction : game.getJunctionIndices()) {
            double distance = game.getShortestPathDistance(node, junction, lastMove);
